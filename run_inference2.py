...
def prepare_audio_data():
    ...
    df_calls.columns = df_calls.iloc[0]
    df_calls = df_calls[1:].reset_index(drop=True)
    

    # Метрики
    df_metrics = pd.read_csv(metrics_csv).rename(columns={"filename": "Файлы"})

    # Объединение
    df = df_calls.merge(df_metrics, on="Файлы", how="left", suffixes=("_x", "_y"))

    # Формируем список выходных колонок
    columns_out = [
        "APPLICATIONID", "ИИН", "Файлы", "Дата и время звонка",
        "Длительность фонограммы", "Направление вызова", "Группа", "credit_rule"
    ]
    mfcc_and_other = [f"mfcc_{i}_{stat}" for i in range(1, 14) for stat in ("mean", "std")]
    mfcc_and_other += [
    ...
    # Удаляем дублирующийся столбец, если есть
    df = df.drop(columns=[col for col in ["APPLICATIONID_y"] if col in df.columns])

    # Оставляем только нужные колонки
    df_out = df[[col for col in columns_out if col in df.columns]].copy()
    ...
    # Инференс
    X_val_proba = df_out[important_features]
    y_val_proba = audio_model.predict_proba(X_val_proba)[:, 1]
    y_val_pred = (y_val_proba >= 0.5).astype(int)

    # Формируем результат
    df_result = df_out.loc[X_val_proba.index, ['APPLICATIONID']].copy()
    df_result.rename(columns={'APPLICATIONID': 'id'}, inplace=True)
    df_result['isFrod'] = y_val_pred
    df_result['probability'] = y_val_proba
...
def main():
    ...
    print(f"DF table data saved:{df.shape}")
    df2 = prepare_audio_data()
    print(f"DF audio data saved:{df2.shape}")
